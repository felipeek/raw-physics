...
static void solve_positional_constraint(Constraint* constraint, r64 h) {
	...
	r64 w1 = e1->inverse_mass + gm_vec3_dot(gm_vec3_cross(r1, n), gm_mat3_multiply_vec3(&e1_inverse_inertia_tensor, gm_vec3_cross(r1, n)));
	r64 w2 = e2->inverse_mass + gm_vec3_dot(gm_vec3_cross(r2, n), gm_mat3_multiply_vec3(&e2_inverse_inertia_tensor, gm_vec3_cross(r2, n)));
...
static void solve_collision_constraint(Constraint* constraint, r64 h) {
	...
	if (d > 0.0) {
		...
		const r64 static_friction_coefficient = 1.0;
...
void pbd_simulate(r64 dt, Entity* entities) {
	...
	for (u32 i = 0; i < NUM_SUBSTEPS; ++i) {
...
#if 0
        int size = array_length(tmp_contacts);
        ...
        Temporary_Contact* abc = array_new(Temporary_Contact);
        for (u32 i = 0; i < array_length(idxs); ++i) {
            array_push(abc, tmp_contacts[idxs[i]]);
        }
        tmp_contacts = abc;
#endif
		...
		for (u32 j = 0; j < array_length(tmp_contacts); ++j) {
			...
			r64 lambda_n = contact->lambda_n;
			r64 lambda_t = contact->lambda_t;
